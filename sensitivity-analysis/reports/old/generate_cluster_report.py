...
Run: python sensitivity-analysis/reports/generate_cluster_report.py
"""
...
def load():
    ...
    data["wv_ca_alloc"] = read(
        p("worldview-sensitivity", "outputs", "cause", "split_credences_cause_areas.csv"))
    data["wv_single_ca"] = read(
        p("worldview-sensitivity", "outputs", "cause", "single_worldview_cause_areas.csv"))

    data["ce_ca_si"] = read(
        p("across-the-board", "outputs", "cause", "cause_area_si.csv"))
    data["ce_ca_alloc"] = read(
        p("across-the-board", "outputs", "cause", "cause_area_allocations.csv"))

    data["dr_ca_index"] = read(
        p("diminishing-returns", "outputs", "cause", "dr_sensitivity_cause_area_index.csv"))
    data["dr_ca_alloc"] = read(
        p("diminishing-returns", "outputs", "cause", "dr_sensitivity_cause_area_allocations.csv"))
    data["ms_ca_index"] = read(
        p("diminishing-returns", "outputs", "cause", "max_spend_cause_area_index.csv"))
    data["ms_ca_alloc"] = read(
        p("diminishing-returns", "outputs", "cause", "max_spend_cause_area_allocations.csv"))
    data["combo_ca_index"] = read(
        p("diminishing-returns", "outputs", "cause", "combo_max_spend_cause_area_index.csv"))
    data["combo_ca_alloc"] = read(
        p("diminishing-returns", "outputs", "cause", "combo_max_spend_cause_area_allocations.csv"))

    # Aggregation-methods cause SI now lives in the unified fund-level index file
    # (column `ca_sensitivity_index`); cause_area_index.csv was retired.
    data["agg_fund_index"] = read(
        p("aggregation-methods", "outputs", "fund", "split_credences_index.csv"))
    data["agg_ca_alloc"] = read(
        p("aggregation-methods", "outputs", "cause", "split_credences_cause_areas.csv"))
    data["agg_single_ca"] = read(
        p("aggregation-methods", "outputs", "cause", "method_cause_areas.csv"))

    data["disc_ca_si"]   = read(
        p("time-discounts", "outputs", "cause", "discount_cause_area_si.csv"))
    data["disc_ca_alloc"]= read(
        p("time-discounts", "outputs", "cause", "discount_cause_area_allocations.csv"))
...
def _build_agg_ca_main(agg_fund_index, agg_ca_alloc, bl_ca):
    """Reconstruct the per-scenario cause-area view of aggregation-method tests.

    Pulls ca_sensitivity_index + credences from the unified fund-level index
    (which carries both fund- and cause-level SI), and computes most_affected_cause /
    most_affected_delta from the cause-area allocations vs baseline."""
    idx = agg_fund_index[~agg_fund_index["bound"].isin(["baseline"])].copy()
    ca_alloc = agg_ca_alloc[~agg_ca_alloc["bound"].isin(["baseline"])].copy()
    ca_delta = ca_alloc[["scenario"]].copy()
    for ca in CAUSE_AREAS:
        ca_delta[f"{ca}_delta"] = ca_alloc[ca] - bl_ca[ca]
    delta_cols = [f"{ca}_delta" for ca in CAUSE_AREAS]
    ca_delta["most_affected_cause"] = (
        ca_delta[delta_cols].abs().idxmax(axis=1).str.replace("_delta", "", regex=False)
    )
    ca_delta["most_affected_delta"] = ca_delta.apply(
        lambda r: r[f"{r['most_affected_cause']}_delta"], axis=1)

    merged = pd.merge(
        idx[["scenario", "method", "bound", "credence_base", "credence_scenario",
             "ca_sensitivity_index"]].rename(
            columns={"ca_sensitivity_index": "sensitivity_index"}),
        ca_delta[["scenario", "most_affected_cause", "most_affected_delta"]],
        on="scenario",
    )
    cred_change_pp = (merged["credence_scenario"] - merged["credence_base"]).abs() * 100
    merged["scaled_SI"] = merged["sensitivity_index"] / cred_change_pp
    merged.loc[cred_change_pp < 1e-9, "scaled_SI"] = pd.NA
    return merged
...
def build_doc(d):
    ...
    # Aggregation-methods cause-area view: assemble from unified fund index
    # (carries ca_sensitivity_index + credences per scenario) and the cause-area
    # allocations (used to derive most_affected_cause vs baseline).
    agg_main = _build_agg_ca_main(d["agg_fund_index"], d["agg_ca_alloc"], bl_ca)
    ...
    # ═══════════════════════════════════════════════════════════
    add_heading(doc, "Least Sensitive Tests at Cause-Area Level (SI < 2.5)", 1)

    doc.add_paragraph(
        "The following tests produced less than 2.5 pp of cross-cause reallocation. "
    ...
    # ═══════════════════════════════════════════════════════════
    # The cluster report does not re-write combined_si.csv files; the
    # main generate_report.py already produces them.
    fcc.build_fund_vs_cluster_section(doc, section_number=8, write_csvs=False)
...
if __name__ == "__main__":
    ...
    doc = build_doc(d)
    out = os.path.join(SCRIPT_DIR, "sensitivity_cluster_report.docx")
    doc.save(out)
    print(f"Saved: {out}")
