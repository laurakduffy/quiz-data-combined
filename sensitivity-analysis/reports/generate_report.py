...
Run: python sensitivity-analysis/reports/generate_report.py
"""
...
# Thresholds for Section 7 illustrative single-perspective analyses
...
def add_most_affected_from_deltas(df):
    """For an index DataFrame whose per-fund deltas are in {fund}_delta columns,
    populate most_affected_fund and most_affected_delta columns."""
    delta_cols = [c for c in df.columns if c.endswith("_delta")]
    if not delta_cols:
        return df
    most_affected_keys = df[delta_cols].abs().idxmax(axis=1).str.replace("_delta", "", regex=False)
    df["most_affected_fund"] = most_affected_keys
    df["most_affected_delta"] = df.apply(
        lambda r: r[f"{r['most_affected_fund']}_delta"], axis=1
    )
    return df


def build_single_table(single_alloc, baseline_df, base_credence_lookup, label_col):
    """Reconstruct a single-credence (100%) sensitivity table from wide allocations + baseline.

    single_alloc has a `label_col` column (e.g. 'worldview' or 'method') and one column per fund.
    base_credence_lookup is a function name -> base credence (best_guess) or None."""
    base_by_fund = dict(zip(baseline_df["fund"], baseline_df["weighted_combined"]))
    rows = []
    for _, r in single_alloc.iterrows():
        label = r[label_col]
        cred_base = base_credence_lookup(label)
        row = {label_col: label, "credence_base": cred_base}
        si = 0.0
        for f in FUNDS:
            delta = float(r[f]) - float(base_by_fund.get(f, 0))
            row[f"{f}_delta"] = delta
            si += abs(delta)
        si /= 2
        row["sensitivity_index"] = si
        if cred_base is not None and abs(1.0 - cred_base) > 1e-9:
            row["scaled_SI"] = si / (abs(1.0 - cred_base) * 100)
        else:
            row["scaled_SI"] = None
        rows.append(row)
    df = pd.DataFrame(rows)
    add_most_affected_from_deltas(df)
    return df
...
def si_histogram(all_si_records):
    ...
    bins = np.arange(0, max(r[0] for r in all_si_records) + 2.5, 2.5)
    ...
    ax.axvline(HIGHLY_SENSITIVE,   color="red",     linestyle="--", linewidth=1.2,
               label=f"High threshold (SI={HIGHLY_SENSITIVE})")
...
def max_si_bar(max_si_per_dim):
    ...
    ax.axvline(HIGHLY_SENSITIVE,   color="red",     linestyle="--", linewidth=1,
               label=f"SI = {HIGHLY_SENSITIVE} (high)")
...
def load():
    ...
    data["ra_summary"] = pd.read_csv(
        p("risk-aversion", "outputs", "fund", "risk_aversion_summary.csv"))
    data["ra_by_fund"] = pd.read_csv(
        p("risk-aversion", "outputs", "fund", "risk_aversion_by_fund.csv"))

    data["wv_index"] = pd.read_csv(
        p("worldview-sensitivity", "outputs", "fund", "split_credences_index.csv"))
    add_most_affected_from_deltas(data["wv_index"])
    data["wv_single_alloc"] = pd.read_csv(
        p("worldview-sensitivity", "outputs", "fund", "single_worldview_allocations.csv"),
        encoding="utf-8")
    with open(p("worldview-sensitivity", "worldview_credences.json"), encoding="utf-8") as fh:
        data["wv_credences"] = json.load(fh)

    data["ce_si"] = pd.read_csv(
        p("across-the-board", "outputs", "fund", "ce_multiplier_si.csv"))
    data["ce_alloc"] = pd.read_csv(
        p("across-the-board", "outputs", "fund", "ce_multiplier_allocations.csv"))

    data["dr_index"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "dr_sensitivity_index.csv"))
    data["dr_alloc"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "dr_sensitivity_allocations.csv"))
    data["ms_index"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "max_spend_sensitivity_index.csv"))
    data["ms_alloc"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "max_spend_sensitivity_allocations.csv"))
    data["combo_index"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "combo_max_spend_index.csv"))
    data["combo_alloc"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "combo_max_spend_allocations.csv"))
    data["combo_by_fund"] = pd.read_csv(
        p("diminishing-returns", "outputs", "fund", "combo_max_spend_by_fund.csv"))

    data["agg_index"] = pd.read_csv(
        p("aggregation-methods", "outputs", "fund", "split_credences_index.csv"),
        encoding="utf-8")
    add_most_affected_from_deltas(data["agg_index"])
    data["agg_method_alloc"] = pd.read_csv(
        p("aggregation-methods", "outputs", "fund", "method_allocations.csv"),
        encoding="utf-8")
    data["agg_methods_cfg"] = json.load(open(
        p("aggregation-methods", "agg_methods_sensitivity.json"), encoding="utf-8"))

    data["ra_cause_area"] = pd.read_csv(
        p("risk-aversion", "outputs", "cause", "risk_aversion_cause_area_summary.csv"))

    data["disc_fund_si"]    = pd.read_csv(
        p("time-discounts", "outputs", "fund", "discount_fund_si.csv"))
    data["disc_fund_alloc"] = pd.read_csv(
        p("time-discounts", "outputs", "fund", "discount_fund_allocations.csv"))
    data["mw_overall_si"]   = pd.read_csv(
        p("moral-weights", "outputs", "fund", "moral_weights_overall_si.csv"))
    data["mw_overall_alloc"]= pd.read_csv(
        p("moral-weights", "outputs", "fund", "moral_weights_overall_allocations.csv"))
    data["mw_ranked"]       = pd.read_csv(
        p("moral-weights", "outputs", "fund", "moral_weights_ranked_summary.csv"))
...
def build_doc(d):
    ...
    doc.add_paragraph(
        ...
        "analyses, sorted by SI. Risk-aversion scenarios (Section 7) are excluded "
    ...
    wv_single = build_single_table(
        d["wv_single_alloc"], d["baseline"],
        lambda name: d["wv_credences"].get(name, {}).get("best_guess"),
        label_col="worldview")

    agg_all  = d["agg_index"].copy()
    agg_main = agg_all[~agg_all["bound"].isin(["baseline"])].copy()
    _agg_cred_by_label = {m["label"]: m["best_guess"] for m in d["agg_methods_cfg"]}
    agg_single = build_single_table(
        d["agg_method_alloc"], d["baseline"],
        lambda name: _agg_cred_by_label.get(name),
        label_col="method")
    ...
    note = doc.add_paragraph(
        ...
        "worldview or aggregation method are presented separately in Section 7 as illustrative "
    ...
    doc.add_paragraph(
        ...
        "Single-worldview (100% credence) scenarios are presented in Section 7."
    )
    ...
    if not top_wv.empty:
        ...
        for _, r in top_wv.iterrows():
            deltas = {f: float(r[f"{f}_delta"]) for f in FUNDS if f"{f}_delta" in r.index}
    ...
    doc.add_paragraph(
        ...
        "Single-method (100% credence) scenarios are presented in Section 7."
    )
    ...
    for _, row in agg_sorted.iterrows():
        deltas = {f: float(row[f"{f}_delta"]) for f in FUNDS if f"{f}_delta" in row.index}
        agg_tests_data.append((row["scenario"], deltas))
    if agg_tests_data:
        add_picture(doc, allocation_change_chart(
            agg_tests_data,
    ...
    # ── 7a. Risk Aversion ────────────────────────────────────────
    add_heading(doc, "7a. Risk Aversion", 2)
    ...
    # ── 7b. 100% Credence in One Worldview ───────────────────────
    add_heading(doc, "7b. 100% Credence in One Worldview", 2)
    ...
    # ── 7c. 100% Credence in One Aggregation Method ──────────────
    add_heading(doc, "7c. 100% Credence in One Aggregation Method", 2)
    ...
    # ═══════════════════════════════════════════════════════════
    # SECTION 8 — FUND-LEVEL vs CLUSTER-LEVEL SENSITIVITY
    # ═══════════════════════════════════════════════════════════
    fcc.build_fund_vs_cluster_section(doc, section_number=8, write_csvs=True)
...
if __name__ == "__main__":
    ...
    doc = build_doc(d)
    out = os.path.join(SCRIPT_DIR, "sensitivity_report.docx")
    doc.save(out)
    print(f"Saved: {out}")
