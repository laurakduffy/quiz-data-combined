...
// Baseline dataset: the website's current dataset (newest config/datasets/*.json),
// same as every other sensitivity analysis. Avoids silently anchoring to a stale
// output_data_median_2M.json (the ATB-2 / DR-4 drift class — see AUDIT_LOG.md).
const basePath = args.base ?? pickDefaultDataset(REPO_ROOT);
...
// ---------------------------------------------------------------------------
// Baseline allocation (canonical) — the suite-wide baseline from the --base
// dataset (median_2M). This is what gets displayed and what --baseline-only
// emits for the cross-suite consistency test. The SI *deltas* are measured
// against a separate CRN reference computed further down.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SI reference baseline
//
// SI deltas are measured against the no-op "baseline" scenario when present:
// it's generated by run_gcr_sensitivity.py at the SAME seed sequence (43..52)
// as every scenario, so common random numbers make the Monte Carlo tail noise
// cancel in the subtraction — a truly-null scenario reads ~0pp instead of a
// spurious value. When it's absent (or --base was given) we fall back to the
// canonical baseline above, which carries the full independent-run MC noise.
// ---------------------------------------------------------------------------

const crnBaselinePath = join(__dirname, 'baseline', 'baseline.json');
const useCrnBaseline = !args.base && existsSync(crnBaselinePath);

let siBaseAllocPct = baseAllocPct;
let siBaseClusterPct = baseClusterPct;

if (useCrnBaseline) {
  const crn = loadDatasetWithClusters(crnBaselinePath);
  const { funding: crnFunding } = computeWeightedAllocation(
    crn.projects,
    worldviews,
    methods,
    totalBudget,
    crn.incrementSize ?? base.incrementSize,
    { drStepSize: crn.drStepSize ?? base.drStepSize }
  );
  siBaseAllocPct = Object.fromEntries(
    fundIds.map((pid) => [pid, totalBudget > 0 ? (crnFunding[pid] / totalBudget) * 100 : 0])
  );
  const crnClusterFunding = clusterFunding(crnFunding, clusters);
  siBaseClusterPct = Object.fromEntries(
    clusterIds.map((cid) => [
      cid,
      totalBudget > 0 ? (crnClusterFunding[cid] / totalBudget) * 100 : 0,
    ])
  );
  console.log('\n  SI deltas: measured against baseline/baseline.json (common random numbers).');
} else {
  console.log(
    `\n  SI deltas: measured against ${basePath.split(/[/\\]/).pop()} ` +
      '(no CRN — generate the "baseline" scenario for noise-cancelled SI).'
  );
}
...
// 'baseline' is the SI reference (not a scenario); 'outputs'/'logs' are infra.
const NON_SCENARIO_DIRS = new Set(['outputs', 'logs', 'baseline']);
const scenarioDirs = readdirSync(__dirname, { withFileTypes: true })
  .filter((d) => d.isDirectory() && !NON_SCENARIO_DIRS.has(d.name))
...
for (const scenarioName of scenarioDirs) {
  ...
  // Deltas (vs the CRN reference when available — see siBase* above)
  const fundDeltas = Object.fromEntries(
    fundIds.map((pid) => [pid, scAllocPct[pid] - siBaseAllocPct[pid]])
  );
  const clusterDeltas = Object.fromEntries(
    clusterIds.map((cid) => [cid, scClusterPct[cid] - siBaseClusterPct[cid]])
  );
