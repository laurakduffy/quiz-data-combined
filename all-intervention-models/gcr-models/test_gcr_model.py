...
# A realistic but fast sweep mirroring the nuclear fund profile.
# n_samples=3000 is large enough for fraction tests while staying fast.
_SWEEP = {
    "r_inf": [1e-10, 1e-7, 1e-3],
    "year_max_risk": [5, 15, 50],
    "year_risk_1pct_max": [20, 100, 200],
    "cumulative_risk_100_yrs": [0.05, 0.10, 0.65],
    "rel_risk_reduction": [1e-4, 5e-4, 2e-3],
    "rate_growth": [0.005, 0.01, 0.04],
    "cubic_growth": {"values": [False, True], "p": [0.90, 0.10]},
    "T_c": {"values": [500, 300, 80], "p": [0.6, 0.3, 0.1]},
    "s": [0.01, 0.1],
    "carrying_capacity_multiplier": {"values": [1.5, 5.0, 100.0], "p": [0.6, 0.3, 0.1]},
}
...
def _run(p_zero=0.0, p_harm=0.0, harm_multiplier=1.0, sweep=None, fixed=None,
         n_samples=_N, seed=42):
    return run_monte_carlo(
        sweep_params=sweep or _SWEEP,
...
def test_effect_fractions(p_zero, p_harm):
    """Harm and positive fractions must be within 1.5pp of targets.

    Zero fraction has a wider tolerance: ~1/9 of the generic _SWEEP parameter
    combinations have (year_max_risk=50, year_risk_1pct_max=20), where the
    intervention window (years 0–15) has essentially zero overlap with the
    narrow gaussian risk peak at year 50.  These samples legitimately compute
    to EV=0 in float64, so the observed zero fraction can exceed p_zero by up
    to ~11% of the non-zero-assigned fraction.
    """
    tol = 0.015
    # ~1/9 of the generic _SWEEP parameter combinations have (year_max_risk=50,
    # year_risk_1pct_max=20), where the intervention window has near-zero overlap
    # with the gaussian risk peak.  These samples compute to EV=0 even when
    # positive- or harm-assigned, so they inflate the zero fraction and deflate
    # the harm fraction by up to 1/9 of their respective assigned fractions.
    mismatch_frac = 1.0 / 9.0
    zero_tol = tol + (1.0 - p_zero) * mismatch_frac   # zeros can run high
    harm_low_tol = tol + p_harm * mismatch_frac        # harm can run low
...
def test_survival_diff_nonzero_low_r_inf():
    ...
    sweep = {
        ...
        "cumulative_risk_100_yrs": [0.10],
        "rel_risk_reduction": [5e-4],
        "rate_growth": [0.01],
        "cubic_growth": [False],
        "T_c": [500],
        "s": [0.01],
    ...
    r = _run(p_zero=0.0, p_harm=0.0, sweep=sweep, n_samples=100)
    tv = r["total_values"]
    assert np.mean(tv) > 0, (
        "Mean EV is zero for r_inf=1e-10 — likely numerical cancellation in "
        "diff_in_survival (float32 regression)."
    )
    assert np.all(tv >= 0), "All samples should be non-negative with p_harm=0, p_zero=0."


def test_survival_diff_nonzero_across_r_inf():
    """Each r_inf tier should contribute non-zero EV on average."""
    for r_inf_val in [1e-10, 1e-7, 1e-3]:
        sweep = {**_SWEEP, "r_inf": [r_inf_val]}
        r = _run(p_zero=0.0, p_harm=0.0, sweep=sweep, n_samples=500)
...
def test_monotonicity_rel_risk_reduction():
    ...
    for rr in [1e-5, 1e-4, 1e-3]:
        sweep = {**_SWEEP, "rel_risk_reduction": [rr]}
        r = _run(p_zero=0.0, p_harm=0.0, sweep=sweep, n_samples=1000, seed=0)
...
def test_zero_rel_risk_reduction():
    ...
    sweep = {**_SWEEP, "rel_risk_reduction": [0.0]}
    r = _run(p_zero=0.0, p_harm=0.0, sweep=sweep, n_samples=300)
...
def test_no_negatives_without_harm():
    ...
    r = _run(p_zero=0.0, p_harm=0.0, n_samples=_N)
    assert np.all(r["total_values"] >= 0), (
        "Unexpected negative EV samples when p_harm=0, p_zero=0."
    )
...
def test_total_value_equals_sum_of_periods():
    ...
    r = _run(p_zero=0.0, p_harm=0.0, n_samples=200)
    ev = r["ev_per_period"]
...
def test_budget_scaling():
    ...
    With the rel_risk_reduction parameterisation (Option A), budget has no effect
    because the risk reduction is specified independently of cost.  Budget only
    scales EV when using bp_reduction_per_bn (Option C), so this test uses that.
    """
    # Use bp_reduction_per_bn sweep instead of rel_risk_reduction so budget matters.
    sweep = {k: v for k, v in _SWEEP.items() if k != "rel_risk_reduction"}
    sweep["bp_reduction_per_bn"] = [0.5, 1.0, 2.0]
    fixed_no_cause = {k: v for k, v in _FIXED.items() if k != "cause_fraction"}

    kw = dict(p_zero=0.0, p_harm=0.0, n_samples=1000, seed=0, sweep=sweep, fixed=fixed_no_cause)
...
def test_harm_direction():
    ...
    tv = r["total_values"]
    # No sample should be simultaneously harm-assigned (neg) and positive — just
    # check that the distribution has both negative and non-negative values.
...
def test_period_values_non_negative_for_positive_samples():
    ...
    r = _run(p_zero=0.0, p_harm=0.0, n_samples=500)
    ev = r["ev_per_period"]
    for k, v in ev.items():
...
def test_different_period_configurations(periods):
    ...
    r = _run(p_zero=0.0, p_harm=0.0, fixed=fixed, n_samples=300)
...
# ---------------------------------------------------------------------------
# 8. Fund-specific regression tests (float32 cancellation in diff_in_survival)
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("fund_key", ["sentinel", "longview_nuclear", "longview_ai"])
def test_fund_effect_fractions(fund_key):
    """Fund harm fractions must be within ±1.5pp of the configured target.

    Zero fractions get a wider tolerance: fund sweeps include (year_max_risk=50,
    year_risk_1pct_max=20) scenarios where the intervention window has near-zero
    overlap with the risk peak.  These samples legitimately compute to EV=0 even
    when positive-assigned, inflating the zero fraction by up to ~1/9 of the
    non-zero-assigned fraction.

    Regression for the float32 risk-array bug, which inflated zero fractions
    (e.g. ~55.8% for sentinel bio) because float32 precision collapsed
    diff_in_survival to zero for small cause_fraction × rel_risk_reduction.
    """
    profile = FUND_PROFILES[fund_key]
    p_zero = profile["p_zero"]
    ...
    frac_harm = np.sum(tv < 0.0) / n
    assert frac_zero <= p_zero + zero_tol, (
        f"{fund_key} zero fraction {frac_zero:.3f} exceeds p_zero={p_zero} + "
        f"allowance={zero_tol:.3f}. Possible float32 cancellation in diff_in_survival."
    )
    assert frac_zero >= p_zero - tol, (
        f"{fund_key} zero fraction {frac_zero:.3f} is below p_zero={p_zero} by more than {tol}."
    )
    assert frac_harm >= p_harm - harm_low_tol, (
        f"{fund_key} harm fraction {frac_harm:.3f} is below p_harm={p_harm} - "
        f"allowance={harm_low_tol:.3f}."
    )
    assert frac_harm <= p_harm + tol, (
        f"{fund_key} harm fraction {frac_harm:.3f} exceeds p_harm={p_harm} by more than {tol}."
    )


@pytest.mark.parametrize("fund_key", ["sentinel", "longview_nuclear", "longview_ai"])
def test_all_funds_nonzero_ev(fund_key):
    """All funds must produce positive mean EV with p_harm=0, p_zero=0.

    Catches float32 cancellation for any fund with small cause_fraction × rel_risk_reduction.
    """
    profile = FUND_PROFILES[fund_key]
    r = run_monte_carlo(
        sweep_params=profile["sweep_params"],
    ...
    assert np.mean(r["total_values"]) > 0, (
        f"{fund_key}: mean EV is zero — likely float32 cancellation in diff_in_survival "
        f"(risk arrays must remain float64)."
    )
...
def test_conservative_bio_survival_diff_nonzero():
    ...
    params = GCRParams(
        ...
        periods_value=[0, 5, 10, 20, 100, 500],
        T_h=np.full(n, 1e14),
        cumulative_risk_100_yrs=np.full(n, 0.05),          # smallest cumulative risk
